...
func TestGetBroker(t *testing.T) {
	...
	tests := map[string]struct {
		...
		// Resolving the stored name needs the database, but the in-memory cache does not: a
		// returning user must not be sent back to broker selection just because the lookup failed.
		"Uses_cached_broker_on_database_error": {user: "shortuserwithbroker@example.com", closeDB: true, cacheBroker: true, wantBroker: mockBrokerGeneratedID},
	}
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
...
func TestEndSessionUserAliasLease(t *testing.T) {
	dbDir := t.TempDir()
	err := db.Z_ForTests_CreateDBFromYAML(
		filepath.Join("testdata", "TestIsAuthenticated", "cache-with-short-username-user.db"), dbDir)
	require.NoError(t, err)
	...
	// Authenticating renames the user to their full name and leaves an alias behind so that the
	// rest of the PAM stack can still resolve the name the user typed.
	iaResp, err := service.IsAuthenticated(context.Background(), &authd.IARequest{
		SessionId:          sessionID,
		AuthenticationData: &authd.IARequest_AuthenticationData{},
	})
	require.NoError(t, err)
	leaseID := iaResp.GetUserAliasLeaseId()
	require.NotEmpty(t, leaseID)

	// A transaction that never reached the authentication step must not keep its lease alive: the
	// PAM module has nothing left to release it with once the session is gone.
	retained, err := m.RetainUserAlias("abandoned-lease", "success")
	require.NoError(t, err, "Setup: retaining the alias should not fail")
	require.True(t, retained, "Setup: the alias should still be retainable")
	pam.Z_ForTests_AddSessionLogin(&service, "abandoned-session", "success", "abandoned-lease")
	_, _ = service.EndSession(context.Background(), &authd.ESRequest{SessionId: "abandoned-session"})
	require.False(t, m.HasUserAlias("abandoned-lease"),
		"ending a session that never authenticated should drop its alias lease")

	// The authenticated session ends as soon as authd reports success, but pam_acct_mgmt still has
	// to resolve the alias afterwards, so its lease must survive.
	_, err = service.EndSession(context.Background(), &authd.ESRequest{SessionId: sessionID})
	require.NoError(t, err)
	require.True(t, m.HasUserAlias(leaseID),
		"ending an authenticated session should keep the alias lease for the rest of the PAM stack")

	_, err = service.ReleaseUserAlias(context.Background(), &authd.RUARequest{LeaseId: leaseID})
	require.NoError(t, err)
}
