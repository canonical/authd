...
func (s Service) GetUserByName(ctx context.Context, req *authd.GetUserByNameRequest) (*authd.User, error) {
	...
	// A username without a domain cannot identify a new broker user. Short usernames are only
	// resolved after the fully qualified name has authenticated once, so do not send an unknown
	// short name through the broker pre-check. Apart from being unnecessary, that can block an NSS
	// lookup while the broker tries to resolve a name that authd cannot map yet.
...
func (s Service) userPreCheck(ctx context.Context, username string) (types.UserEntry, error) {
	...
	// the database with the same UID. The record is keyed by the fully qualified username, because
	// two of them can shorten to the same stored name and must not share a temporary UID.
	u.UID, err = s.userManager.RegisterUserPreAuth(fullUsername, u.Name)
	if err != nil {
