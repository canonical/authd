...
func NewManager(ctx context.Context, dbDir, brokersConfPath string, configuredBrokers []string, hideLocalBroker bool, usersConfig users.Config) (m Manager, err error) {
	...
	pamService := pam.NewService(ctx, userManager, brokerManager, &permissionManager, hideLocalBroker)
