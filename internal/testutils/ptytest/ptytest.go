...
func (c *Console) formatHistory() string {
	...
	for i, ix := range c.interactions {
		fmt.Fprintf(&b, "  %d. %s(%q)", i+1, ix.op, ix.detail)
		switch ix.result {
		case resultOK:
			if ix.op == "WaitFor" {
		...
		case resultExitNoMatch:
			b.WriteString(" → FAILED (command exited without matching)")
		}
		b.WriteByte('\n')
	}
	return b.String()
...
func (c *Console) WaitForTimeout(t *testing.T, pattern string, timeout time.Duration) string {
	...
	for {
		...
		if time.Now().After(deadline) {
			...
			require.FailNow(t, fmt.Sprintf("ptytest: WaitFor(%q) timed out after %s", pattern, timeout),
				"--- interaction history ---\n%s--- current terminal screen ---\n%s\n--- text seen since last match ---\n%s",
				c.formatHistory(),
				ProcessRawOutput(content),
...
func (c *Console) WaitForExit(t *testing.T) error {
	...
	case <-time.After(c.opts.timeout):
		require.FailNow(t, fmt.Sprintf("ptytest: WaitForExit timed out after %s", c.opts.timeout))
