...
const allUserColumns = "name, uid, gid, gecos, dir, shell, broker_id, locked, provider_id, full_username"
const publicUserColumns = "name, uid, gid, gecos, dir, shell, broker_id, locked, provider_id, full_username"
const allUserColumnsWithPlaceholders = "name = ?, uid = ?, gid = ?, gecos = ?, dir = ?, shell = ?, broker_id = ?, locked = ?, provider_id = ?, full_username = ?"
...
type UserRow struct {
	...
	// FullUsername is the fully qualified username as returned by the broker. It differs from Name
	// when short usernames are enabled. It is always serialized (no omitempty) because it is the
	// name authd hands back to the broker on subsequent authentications.
	//
	// ProviderID cannot serve this purpose: it is an opaque identifier, so no name can be derived
	// from it, brokers authenticate against the username (and reject a session whose requested
	// username does not match the authenticated one), and it is empty for local users and for users
	// created before brokers reported it.
...
func userByProviderID(db queryable, brokerID, providerID string) (UserRow, error) {
	query := fmt.Sprintf(`SELECT %s FROM users WHERE broker_id = ? AND provider_id = ?`, publicUserColumns)
...
func usersWithPrimaryGroup(db queryable, gid uint32) ([]UserRow, error) {
	query := fmt.Sprintf(`SELECT %s FROM users WHERE gid = ? ORDER BY uid`, publicUserColumns)
