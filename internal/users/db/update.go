...
func handleUserUpdate(db queryable, u UserRow) error {
	...
	// name change was authorised during UpdateUser. Two things authorise it: a matching
	// broker-scoped provider ID, which identifies the user across a username change at the IdP,
	// and a matching fully qualified username, which identifies the user across the rename between
	// its short and fully qualified forms. The latter is the only proof available for users stored
	// before brokers reported a provider ID.
	if existingUser.Name != "" && existingUser.Name != u.Name {
		sameProviderID := u.ProviderID != "" && existingUser.ProviderID == u.ProviderID && existingUser.BrokerID == u.BrokerID
		// A provider ID on both sides settles the question on its own, so a matching fully
		// qualified username only authorises the rename while it is the best proof available.
		// Otherwise a username reused by the IdP for a new account would take over the old one.
		sameFullUsername := u.FullUsername != "" && existingUser.FullUsername == u.FullUsername &&
			(u.ProviderID == "" || existingUser.ProviderID == "" || sameProviderID)
		if !sameProviderID && !sameFullUsername {
		...
		log.Infof(context.TODO(), "Username changed for UID %d from %q to %q (identity confirmed by the broker-scoped provider ID or the fully qualified username)",
