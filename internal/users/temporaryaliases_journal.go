...
func newTemporaryAliasJournal(dbDir string) (*temporaryAliasJournal, error) {
	...
	if journal.records == nil {
		// A JSON null unmarshals into a nil map, which the first add would panic on. Corrupt
		// durable state must not crash the daemon on every restart, so treat it as empty.
		journal.records = make(map[string]temporaryAliasCleanup)
	}
	return journal, nil
}
...
func (j *temporaryAliasJournal) retargetUID(oldUID, newUID uint32) error {
	j.mu.Lock()
	defer j.mu.Unlock()

	previous := make(map[string]temporaryAliasCleanup, len(j.records))
	for name, cleanup := range j.records {
		if cleanup.UID != oldUID {
			continue
		}
		previous[name] = cleanup
		cleanup.UID = newUID
		j.records[name] = cleanup
	}
	if len(previous) == 0 {
	...
	if err := j.saveLocked(); err != nil {
		for name, cleanup := range previous {
			j.records[name] = cleanup
		}
		return err
	}
...
func (j *temporaryAliasJournal) removeForUID(uid uint32) ([]temporaryAliasCleanup, error) {
	j.mu.Lock()
	defer j.mu.Unlock()

	var removed []temporaryAliasCleanup
	for name, cleanup := range j.records {
		if cleanup.UID != uid {
			continue
		}
		removed = append(removed, cleanup)
		delete(j.records, name)
	}
	if len(removed) == 0 {
		return nil, nil
