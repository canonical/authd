...
func TestTemporaryUserAliasesLookupAndRetain(t *testing.T) {
	...
	// See TestTemporaryUserAliasesRetain: a canonical name adds nothing, so the capacity does not
	// apply to it.
	_, ok, err = aliases.lookupAndRetain("newlease", "canonical")
	require.NoError(t, err)
	require.False(t, ok)
...
func TestTemporaryUserAliasesRetain(t *testing.T) {
	...
	// A canonical name is not an alias, so retaining it adds nothing and must not be refused by
	// the capacity: it would reject an ordinary login.
	ok, err = aliases.retain("newlease", "canonical")
	require.NoError(t, err)
	require.False(t, ok)
...
func TestTemporaryUserAliasesForUIDAndProtectAndDiscard(t *testing.T) {
	...
	// Discard name, scoped to the lease that owns it
	lease1Aliases := aliases.forLease("lease1")
	require.Len(t, lease1Aliases, 1)
	require.Equal(t, "alias1", lease1Aliases[0].name)

	aliases.discardForLease("lease2", "alias1")
	_, ok := aliases.lookup("alias1")
	require.True(t, ok, "Alias owned by another lease must be kept")

	aliases.discardForLease("lease1", "alias1")
	_, ok = aliases.lookup("alias1")
	require.False(t, ok)
...
func TestTemporaryUserAliasesRetargetAndDiscardForUID(t *testing.T) {
	t.Parallel()
	...
	aliases.retargetUID(1001, 2001)
	require.Len(t, aliases.forUID(2001), 2, "the aliases should follow the user to their new UID")
	require.Empty(t, aliases.forUID(1001))
	require.Len(t, aliases.forUID(1002), 1, "the aliases of another user should be left alone")

	discarded := aliases.discardForUID(2001)
	require.Len(t, discarded, 2)
	_, ok := aliases.lookup("alias1")
	require.False(t, ok)
	_, ok = aliases.lookup("alias3")
	require.True(t, ok, "the alias of another user should be kept")
	require.Equal(t, 0, expired, "discarding must not run the expiry cleanup")
}
