...
func TestTemporaryAliasJournalNullFile(t *testing.T) {
	t.Parallel()

	dbDir := t.TempDir()
	journalPath := filepath.Join(dbDir, temporaryAliasJournalName)
	require.NoError(t, os.WriteFile(journalPath, []byte("null"), 0o600))

	journal, err := newTemporaryAliasJournal(dbDir)
	require.NoError(t, err, "a null journal should load as an empty one")

	var addErr error
	require.NotPanics(t, func() {
		addErr = journal.add(temporaryAliasCleanup{Name: "user"})
	}, "adding to a journal loaded from null must not panic")
...
func TestTemporaryAliasJournalRetargetAndRemoveForUID(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, journal.add(temporaryAliasCleanup{Name: "alias1", UID: 1001, NewName: "user1"}))
	require.NoError(t, journal.add(temporaryAliasCleanup{Name: "alias2", UID: 1001, NewName: "user1"}))
	require.NoError(t, journal.add(temporaryAliasCleanup{Name: "alias3", UID: 1002, NewName: "user2"}))

	require.NoError(t, journal.retargetUID(1001, 2001))
	for _, name := range []string{"alias1", "alias2"} {
		cleanup, ok := journal.get(name)
		require.True(t, ok)
		require.Equal(t, uint32(2001), cleanup.UID, "%q should follow the user to their new UID", name)
	}
	cleanup, ok := journal.get("alias3")
	require.True(t, ok)
	require.Equal(t, uint32(1002), cleanup.UID, "the record of another user should be left alone")

	// The change has to survive a restart, otherwise recovery would look the user up by an UID
	// that no longer exists.
	reloaded, err := newTemporaryAliasJournal(dbDir)
	require.NoError(t, err)
	cleanup, ok = reloaded.get("alias1")
	require.True(t, ok)
	require.Equal(t, uint32(2001), cleanup.UID)

	removed, err := reloaded.removeForUID(2001)
	require.NoError(t, err)
	require.Len(t, removed, 2)
	_, ok = reloaded.get("alias1")
	require.False(t, ok)
	_, ok = reloaded.get("alias3")
	require.True(t, ok, "the record of another user should be kept")
}
