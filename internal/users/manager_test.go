...
// TestUpdateUserShortNameTakenByAuthdGroup checks that an authd group already holding the short
// name makes the user keep their fully qualified one. Selecting the short name anyway would build
// a private group colliding with that group, which fails the update and locks the user out.
func TestUpdateUserShortNameTakenByAuthdGroup(t *testing.T) {
	// UpdateUser locks the process-wide local account database, so this test must not
	// contend with the other short-username update tests.
	...
	// Another user owns an identity provider group named like the short name of our user.
	holder := db.NewUserRow("holder", 2222, 2222, "", "/home/holder", "/bin/bash",
		"broker-id", "holder-provider-id", "holder@example.com")
	holderGroup := db.NewGroupRow("user", 3333, "some-idp-group")
	require.NoError(t, userstestutils.DBManager(m).UpdateUserEntry(holder,
		[]db.GroupRow{holderGroup}, nil), "Setup: could not add the group holder")
	...
	require.NoError(t, err, "UpdateUser should fall back to the fully qualified name")

	got, err := userstestutils.DBManager(m).UserByID(1111)
	require.NoError(t, err, "The user should have been created")
	require.Equal(t, "user@example.com", got.Name, "The user should keep their fully qualified name")
}

// TestUpdateUserShortNameKeptWhenGroupIsTheirOwn checks that the user's own private group does not
// count as a conflict, so a user already stored under the short name keeps it.
func TestUpdateUserShortNameKeptWhenGroupIsTheirOwn(t *testing.T) {
	// UpdateUser locks the process-wide local account database, so this test must not
	// contend with the other short-username update tests.
	...
	require.Equal(t, "user", got.Name, "The user should be stored under the short name")
}

// TestUpdateUserFullUsernameOfAnotherIdentity checks that a fully qualified username is only
// trusted to identify a stored user while no stronger proof contradicts it. An identity provider
// can hand the same username to a new account once the old one is gone, and the new account must
// not take over the stored one.
func TestUpdateUserFullUsernameOfAnotherIdentity(t *testing.T) {
	cfg := users.DefaultConfig
	cfg.UseShortUsernames = true

	authenticate := func(brokerID, providerID string) types.UserInfo {
		return types.UserInfo{
			Name:       "user@example.com",
			Dir:        "/home/user@example.com",
			Shell:      "/bin/bash",
	...
	seed := func(t *testing.T, m *users.Manager, providerID string) {
		t.Helper()

		stored := db.NewUserRow("user", 1111, 1111, "", "/home/user", "/bin/bash",
			"broker-id", providerID, "user@example.com")
		privateGroup := db.NewGroupRow("user", 1111, "user@example.com")
		require.NoError(t, userstestutils.DBManager(m).UpdateUserEntry(stored,
			[]db.GroupRow{privateGroup}, nil), "Setup: could not add the stored user")
	}

	t.Run("Another_provider_ID_of_the_same_broker_is_refused", func(t *testing.T) {
		m := newManagerForTestsWithConfig(t, cfg, t.TempDir(),
			users.WithIDGenerator(&users.IDGeneratorMock{UIDsToGenerate: []uint32{2222}}))
		t.Cleanup(func() { _ = m.Stop() })
		seed(t, m, "provider-id")

		err := m.UpdateUser(authenticate("broker-id", "other-provider-id"))
		require.ErrorContains(t, err, "already used by another user")

		got, err := userstestutils.DBManager(m).UserByID(1111)
		require.NoError(t, err, "The stored user should still exist")
		require.Equal(t, "provider-id", got.ProviderID, "The stored user should keep their identity")
	})

	t.Run("Another_broker_reaches_the_broker_binding_check", func(t *testing.T) {
		m := newManagerForTestsWithConfig(t, cfg, t.TempDir(),
			users.WithIDGenerator(&users.IDGeneratorMock{UIDsToGenerate: []uint32{2222}}))
		t.Cleanup(func() { _ = m.Stop() })
		seed(t, m, "provider-id")

		err := m.UpdateUser(authenticate("other-broker-id", "other-provider-id"))
		require.ErrorContains(t, err, "already bound to broker")
	})

	t.Run("A_user_stored_without_a_provider_ID_is_still_matched", func(t *testing.T) {
		m := newManagerForTestsWithConfig(t, cfg, t.TempDir(),
			users.WithIDGenerator(&users.IDGeneratorMock{UIDsToGenerate: []uint32{2222}}))
		t.Cleanup(func() { _ = m.Stop() })
		seed(t, m, "")

		require.NoError(t, m.UpdateUser(authenticate("broker-id", "provider-id")),
			"UpdateUser should adopt the row the fully qualified username points at")

		got, err := userstestutils.DBManager(m).UserByID(1111)
		require.NoError(t, err, "The stored user should have been updated in place")
		require.Equal(t, "provider-id", got.ProviderID, "The provider ID should have been recorded")
	})
}
...
func TestBrokerAndProviderIDForUserResolvesEveryLoginName(t *testing.T) {
	t.Parallel()

	m := newManagerForTests(t, t.TempDir())
	t.Cleanup(func() { _ = m.Stop() })

	// The user was shortened, so their fully qualified name is not the name they are stored under.
	userRow := db.NewUserRow("user1", 1111, 1111, "", "/home/user1", "/bin/bash",
		"broker-id", "provider-123", "user1@example.com")
	require.NoError(t, userstestutils.DBManager(m).UpdateUserEntry(userRow, nil, nil))

	// An authentication is renaming them back, so the short name only lives on as an alias.
	require.NoError(t, m.Z_ForTests_AddTemporaryAlias("lease", "aliased-user1", 1111,
		"broker-id", "provider-123", "user1@example.com"))

	for _, loginName := range []string{"user1", "user1@example.com", "aliased-user1"} {
		brokerID, providerID, err := m.BrokerAndProviderIDForUser(loginName)
		require.NoError(t, err, "the broker binding should be found through %q", loginName)
		require.Equal(t, "broker-id", brokerID, "the broker should be found through %q", loginName)
		require.Equal(t, "provider-123", providerID,
			"the provider ID should be found through %q, otherwise the broker gets none", loginName)
	}
}
...
func TestCleanupTemporaryAlias(t *testing.T) {
	...
	t.Run("Keep_the_local_groups_of_a_name_that_an_account_owns", func(t *testing.T) {
		destGroupFile := localgroupstestutils.SetupGroupMock(t,
			filepath.Join("testdata", "groups", "local_user_in_group.group"))
		m := newManagerForTests(t, t.TempDir())
		t.Cleanup(func() { _ = m.Stop() })

		t.Cleanup(localentries.Z_ForTests_RestoreDefaultOptions)
		localentries.Z_ForTests_SetPasswdPath(
			filepath.Join("testdata", "passwd", "no_authd_users.passwd"))

		// "user1" is an authd user of its own, so the memberships under that name are theirs and
		// not leftovers of the alias that another user was renamed away from.
		otherUser := db.NewUserRow("user1", 1111, 1111, "", "/home/user1", "/bin/bash",
			"broker", "other-prov", "user1@example.com")
		require.NoError(t, userstestutils.DBManager(m).UpdateUserEntry(otherUser, nil, nil),
			"Setup: could not create the other user")

		renamedUser := db.NewUserRow("user2", 2222, 2222, "", "/home/user2", "/bin/bash",
			"broker", "prov", "user2@example.com")
		require.NoError(t, userstestutils.DBManager(m).UpdateUserEntry(renamedUser, nil, nil),
			"Setup: could not create the renamed user")
		require.NoError(t, m.Z_ForTests_AddJournalCleanup(users.TemporaryAliasCleanupForTests{
			Name:            "user1",
			UID:             2222,
			NewName:         "user2",
			OldBrokerID:     "broker",
			...
			NewBrokerID:     "broker",
			NewProviderID:   "prov",
			NewFullUsername: "user2@example.com",
			LocalGroups:     []string{"localgroup1"},
		}), "Setup: could not journal the cleanup")

		require.NoError(t, m.Z_ForTests_CleanupTemporaryAlias("user1"))

		require.NoFileExists(t, destGroupFile,
			"the cleanup must not remove an account from its own local groups")
	})

	t.Run("Keep_the_local_groups_of_a_name_that_a_local_user_owns", func(t *testing.T) {
		destGroupFile := localgroupstestutils.SetupGroupMock(t,
			filepath.Join("testdata", "groups", "local_user_in_group.group"))
		m := newManagerForTests(t, t.TempDir())
		t.Cleanup(func() { _ = m.Stop() })
		...
		t.Cleanup(localentries.Z_ForTests_RestoreDefaultOptions)
		localentries.Z_ForTests_SetPasswdPath(filepath.Join("testdata", "passwd", "local_user1.passwd"))
		localentries.Z_ForTests_SetUserDBEntries([]types.UserEntry{{Name: "user1", UID: 3333}}, nil)
		...
		require.NoFileExists(t, destGroupFile,
			"the cleanup must not remove a local user from their own local groups")
	})
}

func TestRetireUnusedUserAliasesKeepsOtherLeases(t *testing.T) {
	t.Parallel()

	const (
		shortName    = "authd-retire-collision"
	...
	userRow := db.NewUserRow(fullUsername, uid, uid, "", "/home/"+fullUsername, "/bin/bash",
		"broker-id", "providerid-user", fullUsername)
	privateGroup := db.NewGroupRow(fullUsername, uid, fullUsername)
	require.NoError(t, userstestutils.DBManager(m).UpdateUserEntry(userRow,
		[]db.GroupRow{privateGroup}, nil), "Setup: could not create the existing user")

	// Two PAM transactions of the same user overlap, and both prepare the alias that keeps the
	// current name resolvable across the rename they expect.
	for _, leaseID := range []string{"lease-a", "lease-b"} {
		aliases, err := m.PrepareUserAliases(leaseID, types.UserInfo{
			Name: fullUsername, Dir: "/home/" + fullUsername, Shell: "/bin/bash",
			BrokerID: "broker-id", ProviderID: "providerid-user",
		})
		require.NoError(t, err, "Setup: could not prepare the aliases of %q", leaseID)
		require.Equal(t, []string{fullUsername}, aliases,
			"Setup: %q should hold an alias for the current name", leaseID)
	}

	// The first transaction did not rename the user after all, so its own alias is pointless.
	require.NoError(t, m.RetireUnusedUserAliases("lease-a"))
	require.False(t, m.HasUserAlias("lease-a"),
		"the transaction should have retired its own unused alias")
	require.True(t, m.HasUserAlias("lease-b"),
		"the alias of a transaction that may still rename the user must be kept")
}
...
func TestUpdateUserShortUsernames(t *testing.T) {
	...
	for name, tc := range systemCollisionCases {
		t.Run(name, func(t *testing.T) {
			...
			if tc.existingUser {
				require.NoError(t, m.RetireUnusedUserAliases("lease"))
...
func TestRegisterUserPreauth(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			uid, err := m.RegisterUserPreAuth(user.Name, user.Name)
		...
		t.Run(fmt.Sprintf("Iteration_%d", idx), func(t *testing.T) {
			...
			if doPreAuth {
				...
				preAuth := func(t *testing.T) {
					...
					uid, err := m.RegisterUserPreAuth(userName, userName)
...
func TestConcurrentUserUpdateWithShortUsernames(t *testing.T) {
	t.Parallel()
	...
	fullUsername := func(idx int) string {
		return fmt.Sprintf("authd-test-short-user%d@example.com", idx)
	}
	storedName := func(idx int) string {
		return fmt.Sprintf("authd-test-short-user%d", idx)
	}

	wg := sync.WaitGroup{}
	wg.Add(nIterations)

	// The same identity is reachable under two names once it is shortened, and both of them are
	// accepted at the login prompt. Hammer the manager with them at the same time, so that a
	// resolution that mixes them up shows as a duplicated or a merged account.
	for idx := range nIterations {
		t.Run(fmt.Sprintf("Iteration_%d", idx), func(t *testing.T) {
			t.Parallel()
			...
			if doPreAuth {
				t.Run("Pre_auth", func(t *testing.T) {
					t.Parallel()

					uid, err := m.RegisterUserPreAuth(fullUsername(idx), storedName(idx))
					require.NoError(t, err, "RegisterUserPreAuth should not fail but it did")
					preauthUID.Store(uid)
				})
			}
			...
			userUpdate := func(t *testing.T) {
				...
				err := m.UpdateUser(types.UserInfo{
					Name:       fullUsername(idx),
					UID:        preauthUID.Load(),
					Dir:        "/home-prefixes/" + fullUsername(idx),
					Shell:      "/usr/sbin/nologin",
					BrokerID:   "broker-id",
					ProviderID: fmt.Sprintf("providerid-%d", idx),
				})
				require.NoError(t, err, "UpdateUser should not fail but it did")
			}
			...
			lookup := func(t *testing.T) {
				...
				// Both forms are what a user may type at the login prompt, and the manager has to
				// answer with the same identity for either of them, or not know it yet.
				for _, loginName := range []string{fullUsername(idx), storedName(idx)} {
					stored, full, err := m.NamesForLogin(loginName)
					if errors.Is(err, db.NoDataFoundError{}) {
						continue
					}
					require.NoError(t, err, "NamesForLogin(%q) should not fail but it did", loginName)
					require.Equal(t, storedName(idx), stored,
						"%q must resolve to the stored name of its own identity", loginName)
					require.Equal(t, fullUsername(idx), full,
						"%q must resolve to the full name of its own identity", loginName)
				}
			}

			for i := range userUpdateRetries {
				t.Run(fmt.Sprintf("Update_user%d", i), userUpdate)
				t.Run(fmt.Sprintf("Lookup_user%d", i), lookup)
			}
		})
	}
	...
	t.Run("Database_checks", func(t *testing.T) {
		...
		require.NoError(t, err, "AllUsers should not fail but it did")
		require.Len(t, allUsers, nIterations,
			"Each identity must be stored exactly once, whichever name it was reached by")

		uniqueUIDs := make(map[uint32]string, nIterations)
		for _, u := range allUsers {
			old, ok := uniqueUIDs[u.UID]
			require.False(t, ok, "UID %d is used by both %q and %q", u.UID, u.Name, old)
			uniqueUIDs[u.UID] = u.Name
		}

		for idx := range nIterations {
			byStoredName, err := m.UserByName(storedName(idx))
			require.NoError(t, err, "the user should be stored under their short name")

			byFullUsername, err := m.UserByFullUsername(fullUsername(idx))
			require.NoError(t, err, "the user should still be reachable by their full name")
			require.Equal(t, byStoredName.UID, byFullUsername.UID,
				"both names of %q must lead to the same account", fullUsername(idx))
		}
	})
}
...
func TestDeleteUserDropsTemporaryAliases(t *testing.T) {
	destGroupFile := localgroupstestutils.SetupGroupMock(t,
		filepath.Join("testdata", "groups", "empty_local_group.group"))
	...
	dbDir := t.TempDir()
	m := newManagerForTestsWithConfig(t, shortUsernamesConfig, dbDir,
		users.WithIDGenerator(&users.IDGeneratorMock{UIDsToGenerate: []uint32{5555}}))
	user := types.UserInfo{
		Name: "user1@example.com", Dir: "/home/user1@example.com", Shell: "/bin/bash",
		BrokerID: "broker-id", ProviderID: "providerid-user1",
		Groups: []types.GroupInfo{{Name: "localgroup1"}},
	}
	require.NoError(t, m.UpdateUser(user), "Setup: could not create the user")
	require.NoError(t, m.Stop())

	// Short usernames were turned off again, so the next authentication renames the user back and
	// keeps their short name alive as a temporary alias for the PAM transaction still running.
	m = newManagerForTests(t, dbDir)
	aliases, err := m.PrepareUserAliases("lease", user)
	require.NoError(t, err, "Setup: could not prepare the aliases")
	require.Equal(t, []string{"user1"}, aliases)
	require.NoError(t, m.UpdateUser(user), "Setup: could not rename the user back")
	require.True(t, m.HasUserAlias("lease"), "Setup: the alias should still be held")

	groupData, err := os.ReadFile(destGroupFile)
	require.NoError(t, err)
	require.Contains(t, string(groupData), "localgroup1:x:41:user1@example.com,user1",
		"Setup: the alias should carry the local memberships of the user")

	// Read back what the previous steps wrote, as the running system would.
	localentries.Z_ForTests_SetGroupPath(destGroupFile, destGroupFile)

	require.NoError(t, m.DeleteUser("user1@example.com", false))

	groupData, err = os.ReadFile(destGroupFile)
	require.NoError(t, err)
	require.Equal(t, "localgroup1:x:41:", strings.TrimSpace(string(groupData)),
		"deleting the user must also drop the local memberships held under their alias")
	require.False(t, m.HasUserAlias("lease"),
		"an alias of a deleted user must not stay resolvable")
	require.NoFileExists(t, filepath.Join(dbDir, "temporary-user-aliases.json"),
		"the cleanup journaled for the alias must not outlive the user")
}
...
func TestUserByIDAndName(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.isTempUser {
				tc.uid, err = m.RegisterUserPreAuth("tempuser1@example.com", "tempuser1@example.com")
...
func TestGroupByIDAndName(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.preAuthUser != "" {
				tc.gid, err = m.RegisterUserPreAuth(tc.preAuthUser, tc.preAuthUser)
...
// TestRegisterUserPreAuthSeparatesIdentities checks that two fully qualified usernames shortening
// to the same stored name do not share a pre-authentication record. The record stands for one
// identity until it authenticates, so handing both of them the same temporary UID would let the
// second login inherit the first one's.
func TestRegisterUserPreAuthSeparatesIdentities(t *testing.T) {
	cfg := users.DefaultConfig
	cfg.UseShortUsernames = true
	m := newManagerForTestsWithConfig(t, cfg, t.TempDir(),
		users.WithIDGenerator(&users.IDGeneratorMock{UIDsToGenerate: []uint32{1111, 2222}}))
	...
	first, err := m.UserEntryStoredAs(types.UserEntry{Name: "user@example.com"})
	require.NoError(t, err, "UserEntryStoredAs should not fail")
	second, err := m.UserEntryStoredAs(types.UserEntry{Name: "user@other.com"})
	require.NoError(t, err, "UserEntryStoredAs should not fail")
	require.Equal(t, "user", first.Name, "Both users should be presented under the short name")
	require.Equal(t, "user", second.Name, "Both users should be presented under the short name")

	firstUID, err := m.RegisterUserPreAuth("user@example.com", first.Name)
	require.NoError(t, err, "RegisterUserPreAuth should not fail")
	secondUID, err := m.RegisterUserPreAuth("user@other.com", second.Name)
	require.NoError(t, err, "RegisterUserPreAuth should not fail")
	require.NotEqual(t, firstUID, secondUID, "Each identity should get its own temporary UID")

	// The same identity coming back keeps its record, whatever name it is presented under.
	againUID, err := m.RegisterUserPreAuth("user@example.com", first.Name)
	require.NoError(t, err, "RegisterUserPreAuth should not fail")
	require.Equal(t, firstUID, againUID, "A returning identity should reuse its temporary UID")
}
...
func TestRegisterUserPreAuthWhenLocked(t *testing.T) {
	...
	uid, err := m.RegisterUserPreAuth("locked-user@example.com", "locked-user@example.com")
	require.ErrorIs(t, err, userslocking.ErrLock)
