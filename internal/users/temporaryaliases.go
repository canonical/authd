...
func (a *temporaryUserAliases) retain(leaseID, name string) (bool, error) {
	...
	for _, alias := range a.leases {
		...
		// Re-leasing an existing alias to another transaction adds a lease, so the capacity is
		// only relevant once a matching alias was actually found. Checking it earlier would fail
		// a canonical name, which is not an alias and adds nothing.
...
func (a *temporaryUserAliases) lookupAndRetain(leaseID, name string) (temporaryUserAlias, bool, error) {
	...
	for _, alias := range a.leases {
		...
		// See retain: the capacity only applies once re-leasing is about to add a lease.
		if len(a.leases) >= maxTemporaryUserAliases {
...
func (a *temporaryUserAliases) forLease(leaseID string) []temporaryUserAlias {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *temporaryUserAliases) discardForLease(leaseID, name string) {
	a.mu.Lock()
	defer a.mu.Unlock()

	a.removeLocked(temporaryUserAliasKey(leaseID, name))
}
...
func (a *temporaryUserAliases) retargetUID(oldUID, newUID uint32) {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *temporaryUserAliases) discardForUID(uid uint32) []temporaryUserAlias {
	a.mu.Lock()
	defer a.mu.Unlock()

	var discarded []temporaryUserAlias
	for key, alias := range a.leases {
		if alias.uid != uid {
			continue
		}
		a.removeLocked(key)
		if !slices.ContainsFunc(discarded, func(other temporaryUserAlias) bool {
			return other.name == alias.name
		}) {
