...
// If callers ever need to perform multiple Get/Set operations on the same environment, consider
// refactoring to a struct backed by a map[string]string (constructed from []string, serialisable
// back to []string) to avoid the current O(n) linear scan on every operation.
package envutils
...
func Getenv(env []string, key string) string {
	for _, kv := range env {
		if val, found := strings.CutPrefix(kv, key+"="); found {
			return val
		}
	}
	return ""
