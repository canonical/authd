...
1. Build the source package (and ensure that there are no lintian issues on the source package):
...
## Push the .changes file to the edge PPA {#push-the-.changes-file-to-the-edge-ppa}
...
3. Configure the msentraid broker:

```shell
# ubudev1.onmicrosoft.com
ISSUER_ID=03c73201-ef9e-4182-ae04-0adb51f4a0b6
CLIENT_ID=f7128cd7-b23a-4086-bf5c-7d4ba710e707
sudo sed -i -e "s/<ISSUER_ID>/${ISSUER_ID}/g" -e "s/<CLIENT_ID>/${CLIENT_ID}/g" /var/snap/authd-msentraid/current/broker.conf
```

4. Configure the google broker:

```shell
CLIENT_SECRET=<your client secret>
CLIENT_ID=196049008811-pdi83hs6rim6a9cadh5dppm4ugi3cr4k.apps.googleusercontent.com
sudo sed -i -e "s/<CLIENT_SECRET>/${CLIENT_SECRET}/g" -e "s/<CLIENT_ID>/${CLIENT_SECRET}/g" /var/snap/authd-google/current/broker.conf
```
...
Spreadsheet: [https://docs.google.com/spreadsheets/d/1FV9r-e9M\_Hm\_Se2FAaVGHxnsfd-omsRozW43G0qeI8g/edit?gid=1716960291\#gid=1716960291](https://docs.google.com/spreadsheets/d/1FV9r-e9M_Hm_Se2FAaVGHxnsfd-omsRozW43G0qeI8g/edit?gid=1716960291#gid=1716960291)
...
2. Push the commit  
3. Wait for the CI jobs to finish  
4. Redo the relevant tests, depending on the contents of the changes:  
...
```
   ...
   2. Or, to test with a package closer to the one that will be released, follow the “[Rebuild the source package](#download-the-source-packages-from-the-github-ci)” and “[Push the .changes file to the edge PPA](#push-the-.changes-file-to-the-edge-ppa)” sections again.
...
7. Go to [https://launchpad.net/\~ubuntu-enterprise-desktop/+archive/ubuntu/authd-edge/+packages](https://launchpad.net/~ubuntu-enterprise-desktop/+archive/ubuntu/authd-edge/+packages)  
8. Click "Copy packages" in the top right  
9. Select the authd packages for all currently supported Ubuntu versions  
10. Destination PPA: authd stable  
11. Destination series: The the same series  
12. Copy options: Rebuild the copied sources (because we build for more architectures in the stable PPA than in the edge PPA)
