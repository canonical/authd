...
Test login with CLI using Entra auth and MFA with client secret
    ...
    ...
    ...    The client secret is injected into broker.conf at setup (not baked
    ...    into the snapshot), so the base snapshot stays secret-free for
    ...    public-client flows. This covers the alternate configuration where
    ...    entra_auth stays available without register_device=true because
    ...    Microsoft Graph access comes from the configured application secret
    ...    instead.
