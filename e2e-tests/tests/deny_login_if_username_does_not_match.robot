...
Test that login fails if usernames do not match
    [Documentation]    This test verifies that when attempting to log in with a
    ...    remote user whose username does not match the requested username,
    ...    the login fails, while local users can still access the system.
