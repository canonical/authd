...
Test login with CLI using Entra auth and MFA
    ...
    ...    single available authentication mode (entra_auth), so the user goes
    ...    straight to the password prompt after choosing the provider. After
    ...    successful MFA the Entra password is cached locally; the
    ...    provisioning checks verify that the cached password works for sudo.
