...
func TestVerify(t *testing.T) {
	...
	payload, err := json.Marshal(map[string]any{"tid": testTenant, "upn": "user@example.com", "exp": time.Now().Unix() + 3600})
	require.NoError(t, err)
	...
	t.Run("Nonce rewrite ignores nonce-looking values before nonce key", func(t *testing.T) {
		t.Parallel()
		nonce := "actual-nonce-value"
		served := []byte(`{"typ":"nonce","cty":"JWT","nonce":"` + nonce + `","alg":"RS256","kid":"` + testKID + `"}`)
		signed := []byte(`{"typ":"nonce","cty":"JWT","nonce":"` + hashedNonce(nonce) + `","alg":"RS256","kid":"` + testKID + `"}`)
		tok := signToken(t, key, served, signed, payload)
		require.NoError(t, tokenverify.Verify(tok, testTenant, resolver))
	})
	...
	t.Run("Empty expected tenant fails before resolving key", func(t *testing.T) {
		t.Parallel()
		hdr, err := json.Marshal(map[string]any{"alg": "RS256", "kid": testKID})
		require.NoError(t, err)
		tok := signToken(t, key, hdr, hdr, payload)
		resolverCalled := false
		err = tokenverify.Verify(tok, "", func(kid string) (*rsa.PublicKey, error) {
			resolverCalled = true
			return resolver(kid)
		})
		require.Error(t, err)
		require.Contains(t, err.Error(), "expectedTenantID")
		require.False(t, resolverCalled, "empty tenant configuration should fail before fetching keys")
	})
	...
	t.Run("Missing exp fails", func(t *testing.T) {
		...
		hdr, err := json.Marshal(map[string]any{"alg": "RS256", "kid": testKID})
		require.NoError(t, err)
		missingExpPayload, err := json.Marshal(map[string]any{"tid": testTenant})
		require.NoError(t, err)
		tok := signToken(t, key, hdr, hdr, missingExpPayload)
		require.Error(t, tokenverify.Verify(tok, testTenant, resolver))
	})
