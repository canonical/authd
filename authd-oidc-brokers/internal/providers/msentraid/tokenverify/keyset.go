...
// nil, a client with a 10-second timeout is used so a hung JWKS endpoint
// cannot stall the login path indefinitely.
...
func (r *RemoteKeySet) fetch(ctx context.Context) error {
	...
	// Limit the response body to 1 MiB to prevent OOM from a malformed or
	// hostile JWKS response.
	var doc struct {
		Keys []struct {
			Kid string `json:"kid"`
			Kty string `json:"kty"`
