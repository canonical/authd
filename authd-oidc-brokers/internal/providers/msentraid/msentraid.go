...
type Provider struct {
	...
	graphAccessTokenAcquirer graphAccessTokenAcquirer
...
func (p *Provider) acquireGraphAccessToken(
	...
	if p.graphAccessTokenAcquirer != nil {
		return p.graphAccessTokenAcquirer(ctx, clientID, tenantID, token, data)
	}
	return himmelblau.AcquireAccessTokenForGraphAPI(ctx, clientID, tenantID, token, data)
}

// acquireGraphAccessTokenWithRetry retries AADSTS50155 once so a single
// device-authentication failure does not immediately trigger re-enrollment.
// The retry is cancelled with the request.
...
// (e.g. an administrator deleted the device object). A live tenant validation
// confirmed that administrator-deletion case, but the result is not a
// universal mapping for every tenant or authentication operation. See the
// caveats documented on that error. This is the sole case mapped to
...
// app-configuration error), and any unclassified himmelblau.TokenAcquisitionError
// (an unrecognized AADSTS code, or any other acquisition failure, e.g. a
// transient one) -- is returned as a plain error instead. A plain error makes
