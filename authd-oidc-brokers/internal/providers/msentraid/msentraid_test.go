...
// ErrMissingClientCredentials (AADSTS7000218) and any unclassified
// himmelblau.TokenAcquisitionError (unknown/future AADSTS codes, or any other
// acquisition failure) -- must NOT be classified as one, since that would
...
func TestClassifyGraphTokenAcquisitionError(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Unclassified_token_acquisition_error_is_a_plain_error": {
			err: himmelblau.TokenAcquisitionError{},
		},
...
func TestAcquireGraphAccessTokenWithRetry(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"Transient_device_authentication_failure_retries": {
		...
		"Persistent_device_authentication_failure_is_returned": {
			errs:      []error{himmelblau.ErrDeviceAuthenticationFailed, himmelblau.ErrDeviceAuthenticationFailed},
...
func TestGetGroupsRetriesTransientDeviceAuthenticationFailure(t *testing.T) {
	...
	require.Equal(t, int32(2), acquireCalls.Load(), "Graph token acquisition should be retried once")
	require.ElementsMatch(t, []info.Group{
		{Name: "group1", UGID: "id1"},
		{Name: "group2", UGID: "id2"},
	}, got)
