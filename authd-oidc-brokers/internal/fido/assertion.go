...
var (
	...
	// ErrPINBlocked means PIN authentication is temporarily blocked until the
	// device is reinserted or power-cycled; the assertion cannot proceed.
	ErrPINBlocked = errors.New("security key PIN is blocked")
	// ErrPINResetRequired means the device's PIN is hard-blocked and the
	// authenticator must be reset before the assertion can proceed. Resetting
	// the authenticator erases its stored credentials.
	ErrPINResetRequired = errors.New("security key PIN requires an authenticator reset")
	// ErrPINChangeRequired means the device will not issue a PIN token until
	// its PIN is changed, so no PIN the user can enter here will work.
