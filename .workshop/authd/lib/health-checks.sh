...
authd_run_workshop_login_shell() {
    ...
    if [ "$(id -un)" = workshop ]; then
        bash -lc "source /project/.workshop/authd/lib/env.sh; ${script}"
        return
    fi

    sudo -u workshop --login bash -lc "source /project/.workshop/authd/lib/env.sh; ${script}"
}
...
authd_check_pam_config() {
    grep -q 'pam_authd_exec\.so' /etc/pam.d/common-auth &&
    grep -q 'pam_authd_exec\.so' /etc/pam.d/common-account &&
    grep -q 'pam_authd_exec\.so' /etc/pam.d/common-password &&
...
authd_broker_count() {
    if [ ! -d /etc/authd/brokers.d ]; then
        printf '0\n'
        return 0
    fi

    find /etc/authd/brokers.d -maxdepth 1 -name '*.conf' 2>/dev/null | wc -l
}
...
authd_print_validate_report() {
    ...
    if [ "$broker_count" -eq 0 ]; then
        ...
        printf '  \033[1;33m│                                    --client-secret SECRET       │\033[0m\n'
