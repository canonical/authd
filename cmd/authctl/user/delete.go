...
const warningMessage = `Warning: Deleting a user that still owns files on the filesystem can lead
to security issues.
...
func init() {
	...
	deleteCmd.Flags().BoolVarP(&deleteUserRemoveHome, "remove-home", "r", false, "Remove the user's home directory")
}
